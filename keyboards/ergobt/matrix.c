...
static matrix_row_t read_cols(uint8_t row)
{
    if (row < 6) {
        if (mcp23018_status) { // if there was an error
            ...
            mcp23018_status = i2c_write(GPIOB, ERGOBT_I2C_TIMEOUT);             if (mcp23018_status) goto out;
            mcp23018_status = i2c_start(I2C_ADDR_READ, ERGOBT_I2C_TIMEOUT);     if (mcp23018_status) goto out;
            data = i2c_read_nack(ERGOBT_I2C_TIMEOUT);      if (mcp23018_status) goto out;
            data = ~data;
...
static void select_row(uint8_t row)
{
    if (row < 6) {
        ...
        if (mcp23018_status) { // if there was an error
            ...
            mcp23018_status = i2c_start(I2C_ADDR_WRITE, ERGOBT_I2C_TIMEOUT);      if (mcp23018_status) goto out;
            mcp23018_status = i2c_write(GPIOA, ERGOBT_I2C_TIMEOUT);               if (mcp23018_status) goto out;
